...
SELECT CONCAT( mem.firstname, ' ', mem.surname ) AS member_name, fac.name
FROM Members mem
...
INNER JOIN Facilities fac
ON fac.facid = book.facid
WHERE book.starttime LIKE '2012-09-14%' AND CASE WHEN mem.memid = 0 THEN fac.guestcost * book.slots
...
Facilities fac
ON fac.facid = book.facid
WHERE CASE WHEN mem.memid = 0 THEN fac.guestcost * book.slots
ELSE fac.membercost * book.slots END >= 30
ORDER BY cost DESC
